...
void Init_cerchio(float * __restrict h_T, const int NX, const int NY);   // init cerchio
...
//   float msecTotal;
  ...
  fprintf (stderr,"# NX=%d, NY=%d, MAX_ITER=%d , Blocksize_X=%d, Blocksize_Y=%d \n", NX, NY, MAX_ITER, BLOCK_SIZE_X, BLOCK_SIZE_Y);
...
/* Inizializzare un cerchio     */
/********************************/
void Init_cerchio(float * __restrict h_T, const int NX, const int NY)
{
    int i,j;

    for(i=0; i<NX; i++)
        for(j=0; j<NY; j++)
        // Il raggio del cerchio è 1/5 della griglia

            if((i-NX/2)*(i-NX/2)+(j-NY/2)*(j-NY/2)<(NX/5)*(NX/5))
                h_T[NX*j + i] = 1.0;
            else
                h_T[NX*j + i] = 0.0;
}
