...
/******************************************************/
/* JACOBI ITERATION FUNCTION - GPU with Shared Memory */
/******************************************************/

__global__ void Jacobi_Iterator_GPU_shared(const float *__restrict__ T_old, float *__restrict__ T_new, const int NX, const int NY)
{
    // La dimensione della matrice deve essere almeno BLOCK_SIZE_X + 2 e BLOCK_SIZE_Y + 2
    const int dimensione_shared = (BLOCK_SIZE_X+2) * (BLOCK_SIZE_Y+2);
    __shared__ float T_shared[dimensione_shared];

    // Coordinate locali a shared
    const int i = threadIdx.x + 1;
    const int j = threadIdx.y + 1;

    // Coordinate globali
    const int tidx = blockIdx.x * blockDim.x + threadIdx.x;
    const int tidy = blockIdx.y * blockDim.y + threadIdx.y;

    // Bisogna copiare i dati in shared, bisogna copiare la parte del blocco + i bordi
    T_shared[i + (BLOCK_SIZE_X+2) * j] = T_old[tidx + NX * tidy];

    // Copia i bordi
    if (threadIdx.x == 0) {
        T_shared[i-1 + (BLOCK_SIZE_X+2) * j] = T_old[tidx-1 + NX * tidy];
    }
    if (threadIdx.x == BLOCK_SIZE_X-1) {
        T_shared[i+1 + (BLOCK_SIZE_X+2) * j] = T_old[tidx+1 + NX * tidy];
    }
    if (threadIdx.y == 0) {
        T_shared[i + (BLOCK_SIZE_X+2) * (j-1)] = T_old[tidx + NX * (tidy-1)];
    }
    ...
    // Calcolo nord, sud, est, ovest relativo alla shared
    int E = i+1 + (BLOCK_SIZE_X+2) * j;
    int W = i-1 + (BLOCK_SIZE_X+2) * j;
    int N = i + (BLOCK_SIZE_X+2) * (j+1);
    int S = i + (BLOCK_SIZE_X+2) * (j-1);

    /// Update
    T_new[tidx + NX * tidy] = 0.25 * (T_shared[E] + T_shared[W] + T_shared[N] + T_shared[S]);
}
