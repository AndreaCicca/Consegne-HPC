...
texture<float, cudaTextureType2D, cudaReadModeElementType> tex_T_const;
...
// --- GPU temperature distribution
    ...
    // creazione del canale per il texture binding
...
__global__ void copy_constant(float * __restrict__ T, const float * __restrict__ T_const, const int NX, const int NY)
{
    ...
    // accedo i valori della texture tramite tex2D e i, j
    float T_const_value = tex2D(tex_T_const, i, j);
    if (T_const_value > 0) T[P] = T_const_value;
