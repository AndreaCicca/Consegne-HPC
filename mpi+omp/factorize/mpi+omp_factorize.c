...
// Attiva la Ricezione della comunicazione dagli altri rank
        MPI_Irecv(&block_found, 1, MPI_INT, MPI_ANY_SOURCE, 666, MPI_COMM_WORLD, &request);

        t1=MPI_Wtime();
        
        #pragma omp parallel for shared(skip, flag) private(block_idx)
...
// Test se un e' arrivato un messaggio. Se arrivato esci 
           ...
           if ( flag )   {
                 printf("Task %d/%d - block %d - FOUND by thread %d\n",  MPIrank, MPIsize, block_idx, OMPrank );
                  
                 #pragma omp critical // si potrebbe anche usare atomic
                 skip = true;
                 continue;
           }
             ...
             if ( block_factorize (block_idx))  
               { 
                #pragma omp critical // si potrebbe anche usare atomic
                skip = true;
               // Invia un messaggio a tutti i task (come messaggio mando block_idx)
                for (i = 0; i < MPIsize; i++) 
                { 
