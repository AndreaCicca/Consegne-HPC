...
void omp_Jacobi_Iterator_CPU(float * __restrict T, float * __restrict T_new, const int NX, const int NY); 
...
void print_colormap(float * __restrict h_T);                          // zzzz
...
//    copy_rows(h_T_old, NX, NY);
    ...
    omp_Jacobi_Iterator_CPU(h_T_old, h_T_new, NX, NY);
   ...
   #pragma omp parallel
    { 
      #pragma omp critical
      {
      //fprintf (stderr,"%d, %d, %d, %.3f, %.3f, %.3f, %d \n", NX, NY, MAX_ITER, Tp+Tnp, Tp, Tnp, omp_get_num_threads());
...
/***********************************/
/* JACOBI ITERATION FUNCTION - OMP CPU */
/***********************************/
...
int i,j;
        ...
        #pragma omp parallel for private (j) // righe in parallelo
        for(j=1; j<NY-1; j++)
            for(i=1; i<NX-1; i++) {
