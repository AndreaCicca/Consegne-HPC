...
// stampa tutti i dati della simulazione
   ...
   if (mpi_rank == 0)  h_T_whole  = (float *)calloc(WNX * WNY, sizeof(float));
     ...
     // Prima di cominciare con jacobi in una eventuale implementazione con check pointing
    // si potrebbe caricare da disco lo stato di avanzamento della simulazione
    // se il file esiste
    // if (mpi_rank == 0) load_checkpoint(h_T_whole, WNX, WNY); per esempio
...
// scambio delle righe di bordo  TODO
  MPI_Sendrecv( &(h_T_old[NX*(NY-2)]), NX, MPI_FLOAT, next_rank, tag,  
                &(h_T_old[NX*0]),NX,MPI_FLOAT,prev_rank,tag,
                MPI_COMM_WORLD,MPI_STATUS_IGNORE);
 
  MPI_Sendrecv( &(h_T_old[NX*1]), NX, MPI_FLOAT, next_rank, tag,
                &(h_T_old[NX*(NY-1)]),NX,MPI_FLOAT,prev_rank,tag,
                MPI_COMM_WORLD,MPI_STATUS_IGNORE); 

//    Checkpointing. Ogni simulazione  può disporre di un tempo di calcolo limitato. Può essere utile al possibilità 
//    di salvare su disco lo stato di avanzamento della simulazione in modo da poterlo riprendere in tempi successivi

//    if (mpi_rank == 0 ) print_colormap(h_T_old,NX,NY); // stampa della matrice di stato

// riduzione dell'overhead di comunicazione attraverso la sovrapposizione comunicazione /computazione
...
// invio dei sottodomini a rank 0 TODO
// receive nel master tramite MPI_GATHER
...
/***********************************/
/* JACOBI ITERATION EXTERNAL FUNCTION - CPU */
/***********************************/
void Jacobi_Iterator_CPU_external(float * __restrict T, float * __restrict T_new, const int NX, const int NY)
{
int i,j;
